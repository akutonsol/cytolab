  ...
  for await (const batch of legacy.stream<LegacyRecordLite>('record', { incremental: ctx.incremental })) {
    ...
    for (const rec of batch) {
      ...
      if (pivot.gyn) {
        ...
        if (!ctx.dryRun) {
          await prisma.gynClinicalFeatures.upsert({ where: { recordId }, create: data, update: pivot.gyn });
        }
        gynCount++;
      } else if (pivot.nonGyn) {
        const data = { id: await idMap.getOrCreate('nonGynFeatures', rec.id), labId, recordId, ...pivot.nonGyn };
        if (!ctx.dryRun) {
          await prisma.nonGynClinicalFeatures.upsert({ where: { recordId }, create: data, update: pivot.nonGyn });
        }
        nonGynCount++;
      }
    }

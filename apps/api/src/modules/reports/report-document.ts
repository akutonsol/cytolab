...
// ─── Small builders ───────────────────────────────────────────────────────────
const sectionLabel = (text: string, topMargin = 8): Content => ({
  text,
  fontSize: 8.5,
  bold: true,
  color: INDIGO,
...
const infoRow = (label: string, value?: string | null, opts: { bold?: boolean; color?: string } = {}): TableCell[] => {
  ...
  return [
    { text: label, fontSize: 8, color: SLATE_LIGHT },
    {
...
export function buildReportDefinition(data: ReportDocumentData): TDocumentDefinitions {
  ...
  const headerBand: Content = {
    table: {
      ...
      body: [[
        ...
        {
          stack: [
            { text: `Ref  ${reportRef}`, color: INDIGO_ON, fontSize: 9, alignment: 'right' },
    ...
    columns: [
      {
        width: '*',
        stack: [
          sectionLabel('PATIENT INFORMATION', 10),
          infoTable([
      ...
      },
      {
        width: '*',
        stack: [
          sectionLabel('SPECIMEN INFORMATION', 10),
          infoTable([
  ...
  const specimenGrid: Content =
    uniqueSpecimens.length > 0
      ? {
          columns: chunk(uniqueSpecimens, Math.ceil(uniqueSpecimens.length / Math.min(uniqueSpecimens.length, 3))).map((group) => ({
            ...
            stack: group.map((s) => ({
              margin: [0, 0, 0, 6],
              stack: [
                { text: displaySpecimen(s.type), fontSize: 9.5, bold: true, color: SLATE },
                {
      ...
      : { text: 'No specimens recorded.', fontSize: 9.5, color: SLATE_MUTED, italics: true };
    ...
    ? entries.flatMap((entry, i): Content[] => [
        {
          columns: [
            ...
            { width: '*', text: entry.specimenLabel ? displaySpecimen(entry.specimenLabel) : `Specimen ${i + 1}`, fontSize: 9.5, bold: true, color: SLATE },
          ],
          margin: [0, i === 0 ? 4 : 10, 0, 5],
        },
        entry.lines.length
          ? {
              table: {
                ...
                body: [
                  ...
                  ...entry.lines.map((l): TableCell[] => {
                    ...
                    return [
                      { text: l.abbreviation || '—', fontSize: 9.5, bold: true, color: SLATE },
                      {
                      ...
                      { text: l.result || '—', fontSize: 9.5, color: SLATE_MID },
                      { text: abn ? 'Abnormal' : 'Normal', fontSize: 8, bold: true, color: abn ? RED : GREEN },
                    ];
                  }),
                ],
              },
          ...
          : { text: 'No findings recorded', italics: true, fontSize: 9.5, color: SLATE_MUTED, margin: [0, 0, 0, 2] },
      ])
    : [{ text: 'No findings recorded.', italics: true, fontSize: 9.5, color: SLATE_MUTED }];
  ...
  const narrativeSection: Content[] = narrativeText
    ? [
        ...
        accentBox(SUBTLE_BG, INDIGO, [
          { text: narrativeText, fontSize: 9.5, color: SLATE, lineHeight: 1.5, italics: true },
        ]),
      ]
    : [];
  ...
  const diagnosisSection: Content[] = record.clinicalDiagnosis
    ? [
        ...
        accentBox(INDIGO_LIGHT, INDIGO, [
          { text: record.clinicalDiagnosis, fontSize: 11, bold: true, color: SLATE, lineHeight: 1.35 },
        ]),
      ]
    : [];
  ...
  const authLeft: any = authorized
    ? {
        ...
        stack: [
          ...
          ...(authorizer.designation ? [{ text: authorizer.designation, fontSize: 9, color: INDIGO_DARK } as Content] : []),
          { text: `Authorized ${fmtDateTime(authorizer.signedAt)}`, fontSize: 9, color: SLATE_LIGHT, margin: [0, 3, 0, 0] },
        ],
      }
    : {
        width: '*',
  ...
  const signatureInner: Content = authorizer.signatureDataUri
    ? { image: authorizer.signatureDataUri, fit: [180, 44], alignment: 'center', margin: [0, 4, 0, 4] }
    : {
        stack: [
          { text: '________________________', alignment: 'center', color: SLATE_MUTED, fontSize: 12, margin: [0, 14, 0, 2] },
  ...
  const authRight: any = {
    ...
    stack: [
      ...
      {
        table: { widths: ['*'], heights: [54], body: [[{ stack: [signatureInner], border: [true, true, true, true] }]] },
        layout: {
          hLineWidth: () => 1,
          vLineWidth: () => 1,
          hLineColor: () => BORDER,
          ...
          columns: [
            {
              width: '*',
              stack: [
                { text: 'CYTOTECHNOLOGIST', fontSize: 7, bold: true, color: SLATE_MUTED, characterSpacing: 0.5 },
                { text: cytotechnologist || '—', fontSize: 9.5, bold: true, color: SLATE, margin: [0, 3, 0, 0] },
              ],
            },
            {
              width: '*',
              stack: [
                { text: 'COMMENTS', fontSize: 7, bold: true, color: SLATE_MUTED, characterSpacing: 0.5 },
                { text: 'APPROVED', fontSize: 9.5, bold: true, color: GREEN, margin: [0, 3, 0, 0] },
              ],
            },
          ],
          columnGap: 24,
  ...
  return {
    ...
    defaultStyle: { font: 'Helvetica', fontSize: 9.5, color: SLATE, lineHeight: 1.2 },
...
function findingTh(text: string): TableCell {
  return { text, fontSize: 7, bold: true, color: SLATE_MUTED, characterSpacing: 0.5, fillColor: SUBTLE_BG };
}

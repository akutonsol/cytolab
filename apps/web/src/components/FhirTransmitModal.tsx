    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 2300, background: 'rgba(15,23,42,0.55)' }} onClick={onClose}>
      <div className="flex max-h-[88vh] w-full max-w-[640px] flex-col rounded-2xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          <label className="mb-1 block text-[12px] font-semibold uppercase tracking-wide text-[#475569]">Destination endpoint</label>
          ...
          {active.length === 0 && <div className="mt-2 text-[13px] text-[#475569]">No active endpoints — add one on the FHIR page first.</div>}
        ...
        <div className="flex justify-end gap-2 border-t border-slate-200 p-4">
          <button onClick={copyJson} className="flex items-center gap-1.5 rounded-lg border border-[#E2E8F0] px-4 py-2 text-[14px] font-semibold text-[#475569]"><Copy size={15} /> Copy JSON</button>

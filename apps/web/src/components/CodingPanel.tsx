    ...
    <div className="fixed inset-0 flex justify-end" style={{ zIndex: 2200, background: 'rgba(15,23,42,0.55)' }} onClick={onClose}>
      <div className="flex h-full w-full max-w-[600px] flex-col bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {/* Suggested */}
          <div className="mb-2 flex items-center gap-1.5 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]"><Sparkles size={13} className="text-[#7C3AED]" /> Suggested Codes</div>
          {suggestions.length === 0 ? (
            <div className="mb-5 text-[13px] text-[#94A3B8]">No suggestions for this record.</div>
          ) : (
                ...
                return (
                  <div key={`${s.code.id}-${s.codeType}`} className="flex items-center gap-2.5 rounded-xl border border-[#EEF2F7] px-3 py-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="truncate text-[12px] text-[#64748B]">{s.code.display} · <span className="text-[#94A3B8]">{s.reason}</span></div>
                    </div>
                    {done ? (
          ...
          {/* Assigned */}
          <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Assigned Codes ({assigned.length})</div>
          {assigned.length === 0 ? (
            <div className="mb-5 text-[13px] text-[#94A3B8]">No codes assigned yet.</div>
          ) : (
            <div className="mb-5 flex flex-col gap-2">
              {assigned.map((a) => (
                <div key={a.id} className="flex items-center gap-2.5 rounded-xl border border-[#EEF2F7] bg-[#F8FAFC] px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-[12px] text-[#64748B]">{a.code.display}</div>
                  </div>
          ...
          {/* Search & add */}
          <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Search &amp; Add</div>
          ...
          {search.trim() && (
            <div className="flex flex-col gap-1.5">
              {results.length === 0 ? <div className="py-3 text-[13px] text-[#94A3B8]">No matching codes.</div> : results.slice(0, 20).map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-center gap-2.5 rounded-lg border border-[#F1F5F9] px-3 py-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="truncate text-[12px] text-[#64748B]">{c.display}</div>
                    </div>

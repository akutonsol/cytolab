    ...
    return (
      <div className="min-h-full pt-4" style={{ background: '#F8FAFC' }}>
        <EmptyState className="mt-16"
              ...
              description={<>LOINC/SNOMED Coding is disabled for this lab.</>}
            />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-full pb-10 pt-4" style={{ background: '#F8FAFC' }}>
      ...
      <div className="mb-4 flex flex-wrap gap-1 rounded-full bg-[#F1F5F9] p-1" style={{ width: 'fit-content' }}>
        {TABS.map((t) => (
          <button key={t} onClick={() => setTab(t)} className="rounded-full px-4 py-1.5 text-[13px] font-semibold transition-colors" style={tab === t ? { background: '#fff', color: '#0F172A', boxShadow: '0 1px 4px rgba(0,0,0,0.08)' } : { color: '#475569' }}>{t}</button>
        ))}
      </div>
        ...
        <Card radius="md" elevation="soft" border="hairline" className="overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13px]">
              ...
              <tbody>
                {records.length === 0 ? <tr><td colSpan={7} className="px-3 py-12 text-center text-[#475569]">No records to code.</td></tr> : records.map((r) => {
        ...
        <Card radius="md" elevation="soft" border="hairline" className="overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13px]">
              ...
              <tbody>
                {codes.length === 0 ? <tr><td colSpan={6} className="px-3 py-12 text-center text-[#475569]">No codes.</td></tr> : codes.map((c) => (
                  <tr key={c.id} className="border-b border-[#F1F5F9] hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-3 py-2.5 text-[#475569]">{c.category ?? '—'}</td>
                    <td className="px-3 py-2.5 text-[#334155]">{c.usageCount}</td>
                    <td className="px-3 py-2.5">{c.isActive ? <span className="text-[12px] font-semibold text-[#16A34A]">Active</span> : <span className="text-[12px] font-semibold text-[#475569]">Inactive</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div>
              ...
              <div className="flex gap-1.5">
                {(['csv', 'json'] as const).map((f) => <button key={f} onClick={() => setFormat(f)} className="rounded-lg border px-3 py-2 text-[13px] font-semibold uppercase transition-colors" style={format === f ? { borderColor: '#4F46E5', background: '#EEF2FF', color: '#4F46E5' } : { borderColor: '#E2E8F0', color: '#475569' }}>{f}</button>)}
              </div>
            </div>
            ...
            <span className="ml-auto text-[13px] text-[#475569]">{preview?.count ?? 0} coded record{(preview?.count ?? 0) === 1 ? '' : 's'} in range</span>
          </div>

          <div className="mt-5">
            <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-[#475569]">Preview (first 5)</div>
            <div className="flex flex-col gap-2">
              {(preview?.records ?? []).length === 0 ? <div className="text-[13px] text-[#475569]">No coded records in this range.</div> : (preview?.records ?? []).slice(0, 5).map((r, i) => (

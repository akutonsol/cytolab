...
export function DivergingBars({ data, currentMonth, height = 460 }: { data: any[]; currentMonth: string; height?: number }) {
  ...
  const ActualLabel = (p: any) => {
    ...
    return <text x={tx} y={p.y + p.height / 2} dy={4} textAnchor={inside ? 'end' : 'start'} fontSize={12} fontWeight={700} fill={inside ? '#fff' : CHART.ink}>{p.value.toLocaleString()}</text>;
  };
    ...
    <div style={{ width: '100%' }}>
      <ResponsiveContainer width="100%" height={height}>
        <BarChart layout="vertical" data={rows} stackOffset="sign" margin={{ top: 8, right: 24, bottom: 8, left: 8 }} barCategoryGap="26%">
          ...
          <Bar dataKey="negDeficit" stackId="a" fill={DEFICIT_GRAY} radius={[11, 11, 11, 11]} isAnimationActive={false} maxBarSize={24}>
            <LabelList dataKey="deficit" content={DeficitChip} />
          </Bar>
          <Bar dataKey="actual" stackId="a" radius={[11, 11, 11, 11]} isAnimationActive={false} maxBarSize={24} background={{ fill: LANE, radius: 12 } as any}>
    ...
    <div style={{ width: '100%' }}>
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 12, right: 8, bottom: 4, left: 4 }}>
          ...
          <Area yAxisId="r" type="monotone" dataKey="revenue" stroke={CHART.ink} strokeWidth={2} fill="url(#revGrad)"
            dot={{ r: 3.5, fill: '#fff', stroke: CHART.ink, strokeWidth: 2 }} activeDot={{ r: 5, fill: '#fff', stroke: CHART.ink, strokeWidth: 2 }} isAnimationActive={false} />
          <Area yAxisId="v" type="monotone" dataKey="volume" stroke={ACTUAL_BLUE} strokeWidth={2.5} fill="url(#volGrad)"
            dot={{ r: 4, fill: '#fff', stroke: ACTUAL_BLUE, strokeWidth: 2 }} activeDot={{ r: 5.5, fill: '#fff', stroke: ACTUAL_BLUE, strokeWidth: 2 }} isAnimationActive={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
        ...
        <LineChart data={data} margin={{ top: 30, right: 8, bottom: 4, left: 4 }}>
          ...
          <Line type="monotone" dataKey="pct" stroke="url(#compStroke)" strokeWidth={3} isAnimationActive={false}
            dot={(props: any) => {
              ...
              const c = payload.pct >= 60 ? CHART.success : payload.pct >= 40 ? '#f59e0b' : '#4f7df9';
              return <circle key={index} cx={cx} cy={cy} r={4.5} fill="#fff" stroke={c} strokeWidth={2.5} />;
            }} />
        </LineChart>
      </ResponsiveContainer>
    </div>

...
/* ---- Diverging horizontal bars: actual (blue, right) vs deficit (gray, left) ---- */
export function DivergingBars({ data, currentMonth, height = 460 }: { data: any[]; currentMonth: string; height?: number }) {
  ...
  const YTick = (props: any) => {
    ...
    return (
      <text x={props.x - 6} y={props.y} dy={4} textAnchor="end" fontSize={12} fontWeight={cur ? 700 : 500} fill={cur ? CHART.primary : CHART.axis}>
        {props.payload.value}
      </text>
    );
  };
  return (
    <div style={{ width: '100%' }}>
      <ResponsiveContainer width="100%" height={height}>
        <BarChart layout="vertical" data={rows} stackOffset="sign" margin={{ top: 8, right: 24, bottom: 8, left: 8 }} barCategoryGap={10}>
          ...
          <Bar dataKey="negDeficit" stackId="a" fill={CHART.barIdle} radius={[6, 0, 0, 6]} isAnimationActive={false} maxBarSize={16}>
            <LabelList dataKey="deficit" position="left" formatter={(v: any) => (v ? `-${v.toLocaleString()}` : '')} fill={CHART.axis} fontSize={11} />
          </Bar>
          <Bar dataKey="actual" stackId="a" radius={[0, 6, 6, 0]} isAnimationActive={false} maxBarSize={16}>
            {rows.map((r, i) => <Cell key={i} fill={r.current ? CHART.primary : '#9db8fb'} />)}
            <LabelList dataKey="actual" position="right" formatter={(v: any) => (v ? v.toLocaleString() : '')} fill={CHART.ink} fontSize={11} fontWeight={600} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
  ...
  return (
    <div style={{ background: CHART.ink, color: '#fff', borderRadius: 12, padding: '10px 12px', boxShadow: '0 8px 24px rgba(16,24,40,.18)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 600 }}>
        <span style={{ width: 8, height: 8, borderRadius: 999, background: CHART.primary }} /> Volume <span style={{ marginLeft: 'auto' }}>{vol}</span>
      </div>
    ...
    <div style={{ width: '100%' }}>
      <ResponsiveContainer width="100%" height={height}>
        <AreaChart data={data} margin={{ top: 10, right: 8, bottom: 4, left: 4 }}>
          ...
          <Area yAxisId="v" type="monotone" dataKey="volume" stroke={CHART.primary} strokeWidth={2.5} fill="url(#volGrad)" dot={false} activeDot={{ r: 5, strokeWidth: 0 }} isAnimationActive={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
...
/* ---- Weekly TAT compliance line (green on-time / red delayed dots) ---- */
export function ComplianceLine({ week, height = 150 }: { week: any[]; height?: number }) {
  ...
  return (
    <div style={{ width: '100%' }}>
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={data} margin={{ top: 16, right: 8, bottom: 4, left: 4 }}>
          <CartesianGrid vertical={false} stroke={CHART.grid} />
          ...
          <Line type="monotone" dataKey="pct" stroke={CHART.primary} strokeWidth={2.5} isAnimationActive={false}
            dot={(props: any) => {
              const { cx, cy, payload, index } = props;
              if (!payload.has) return <circle key={index} cx={cx} cy={cy} r={3} fill={CHART.track} />;
              const c = payload.pct >= 50 ? CHART.success : CHART.danger;
              return <circle key={index} cx={cx} cy={cy} r={4} fill={c} stroke="#fff" strokeWidth={2} />;
            }} />
        </LineChart>
      </ResponsiveContainer>
    </div>

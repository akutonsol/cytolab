  ...
  return (
    <div className="min-h-full pb-10 pt-4" style={{ background: '#F8FAFC' }}>
      ...
      <div className="relative mx-auto mb-8 max-w-3xl">
        <Search size={20} className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-[#94A3B8]" />
        <input
          value={q}
        ...
        {showResults && (
          <div className={`${CARD} absolute z-20 mt-2 max-h-[420px] w-full overflow-y-auto p-2`}>
            {!search?.results.length ? (
              <div className="px-4 py-6 text-center text-[14px] text-[#94A3B8]">
              ...
              search.results.map((r: KbSearchResult) => (
                <button key={r.id} onClick={() => router.push(`/knowledge-base/articles/${r.slug}`)}
                  ...
                  <div className="mt-0.5 line-clamp-1 text-[13px] text-[#64748B]">{r.excerpt}</div>
                </button>
              ))
            )}
          </div>
      ...
      {/* Category grid */}
      <h2 className="mb-3 text-[15px] font-semibold uppercase tracking-wide text-[#94A3B8]">Browse by category</h2>
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {!categories?.length ? (
          <div className={`${CARD} col-span-full p-8 text-center text-[14px] text-[#94A3B8]`}>No categories yet.</div>
        ) : (
          categories.map((c) => (
            <button key={c.id} onClick={() => router.push(`/knowledge-base/articles?category=${c.id}`)}
              ...
              <div className="min-w-0 flex-1">
                ...
                {c.description && <div className="mt-0.5 line-clamp-2 text-[13px] text-[#64748B]">{c.description}</div>}
                <div className="mt-2 text-[12px] font-medium text-[#94A3B8]">{c._count?.articles ?? 0} article{(c._count?.articles ?? 0) === 1 ? '' : 's'}</div>
              </div>
            </button>
          ))
        )}
      ...
      {!!pinned?.length && (
        <div className="mb-8">
          <h2 className="mb-3 flex items-center gap-1.5 text-[15px] font-semibold uppercase tracking-wide text-[#94A3B8]"><Pin size={14} /> Pinned</h2>
          <div className="flex gap-4 overflow-x-auto pb-2">
            {pinned.map((a) => (
              <button key={a.id} onClick={() => router.push(`/knowledge-base/articles/${a.slug}`)}
                ...
                {a.excerpt && <div className="mt-1 line-clamp-2 text-[13px] text-[#64748B]">{a.excerpt}</div>}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Recent */}
      <div>
        <h2 className="mb-3 flex items-center gap-1.5 text-[15px] font-semibold uppercase tracking-wide text-[#94A3B8]"><Clock size={14} /> Recently published</h2>
        <div className={`${CARD} divide-y divide-[#F1F5F9]`}>
          {!recentFive.length ? (
            <div className="p-8 text-center text-[14px] text-[#94A3B8]">No published articles yet.</div>
          ) : (
            recentFive.map((a) => (
              <button key={a.id} onClick={() => router.push(`/knowledge-base/articles/${a.slug}`)}
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-[12px] text-[#94A3B8]">{a.category?.name ?? 'General'} · {a.viewCount} view{a.viewCount === 1 ? '' : 's'}</div>
                </div>

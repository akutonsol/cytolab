...
export default function ConsultDetailPage() {
  ...
  if (!isEnabled('TELECONSULTATION')) return <div className="grid h-[60vh] place-items-center text-[#64748B]">Teleconsultation is disabled for this lab.</div>;
  if (isLoading || !c) return <div className="grid h-[60vh] place-items-center text-[#94A3B8]">Loading…</div>;
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-4 lg:col-span-2">
          <div className={`${CARD} p-5`}>
            <div className="mb-3 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Case Details (de-identified)</div>
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-[13px]">
              <div><div className="text-[11px] font-semibold uppercase tracking-wide text-[#94A3B8]">Patient</div><div className="mt-0.5 font-semibold text-[#0F172A]">{c.patientInitials}</div></div>
            ...
            <div className="mt-3">
              <div className="text-[11px] font-semibold uppercase tracking-wide text-[#94A3B8]">Specific Question</div>
          ...
          {/* Timeline */}
          <div className={`${CARD} p-5`}>
            <div className="mb-4 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Progress</div>
              ...
              {TIMELINE.map((stage, i) => (
                <div key={stage} className="flex flex-1 items-center last:flex-none">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-[11px] font-semibold" style={{ color: i <= idx ? '#4F46E5' : '#94A3B8' }}>{stage}</span>
                  </div>
          ...
          {c.consultantResponse ? (
            <div className={`${CARD} p-5`}>
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Consultant Response</div>
              ...
              <div className="mt-2 text-[12px] text-[#94A3B8]">Received {dateTime(c.respondedAt)}</div>
              ...
              {c.status === 'Responded' && (
                <div className="mt-4 flex gap-2 border-t border-[#F1F5F9] pt-4">
                  ...
                  <button onClick={() => act.mutate('decline')} className="rounded-lg border border-[#E2E8F0] px-4 py-2 text-[13px] font-semibold text-[#64748B]">Decline</button>
                </div>
              )}
            </div>
          ) : (
            <div className={`${CARD} p-5 text-[14px] text-[#94A3B8]`}>Awaiting the consultant's response.</div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className={`${CARD} p-5`}>
            <div className="mb-3 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Consultant</div>
            <div className="text-[15px] font-bold text-[#0F172A]">{c.consultantName}</div>
            {c.consultantInstitution && <div className="text-[13px] text-[#64748B]">{c.consultantInstitution}</div>}
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 text-[13px]">
              <div><div className="text-[11px] font-semibold uppercase tracking-wide text-[#94A3B8]">Urgency</div><div className="mt-0.5 font-semibold" style={{ color: URGENCY_META[c.urgency].fg }}>{c.urgency}</div></div>
              <div><div className="text-[11px] font-semibold uppercase tracking-wide text-[#94A3B8]">Due</div><div className="mt-0.5" style={{ color: c.isOverdue ? '#B91C1C' : '#334155' }}>{shortDate(c.dueDate)}</div></div>
            </div>
          </div>

          <div className={`${CARD} p-5`}>
            <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Shared Materials</div>
            {shared.length === 0 ? <div className="text-[13px] text-[#94A3B8]">Case summary only.</div> : (
          ...
          <div className={`${CARD} p-5`}>
            <div className="mb-2 text-[12px] font-bold uppercase tracking-wide text-[#94A3B8]">Access Link</div>
            <div className="flex items-center gap-2">
              <input readOnly value={publicLink} className="h-9 flex-1 rounded-lg border border-[#E2E8F0] bg-[#F8FAFC] px-2 text-[12px] text-[#64748B] outline-none" />
            ...
            <div className="mt-1.5 text-[11px] text-[#94A3B8]">Expires {shortDate(c.tokenExpiresAt)}</div>

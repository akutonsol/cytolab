...
| **Status** | ✅ **CLOSED** — **R-016a CLOSED** (active SYSTEM write path restored) + **R-016b CLOSED** (sealed-generation monitor architecture). The frozen generation-0 is now classifiable as `SEALED` against a full-generation snapshot; the monitor no longer reports a false `COMPROMISED` for a properly sealed frozen generation. |
...
| **Resolution — R-016b** | A **sealed generation** is a frozen prior generation whose integrity invariant is "unchanged since sealing" (not live-chain verifiability). New append-only `AuditChainSeal` model (Option A) carries `{eventCount, terminalSequence, terminalSelfHash, snapshotDigest}`; `snapshotDigest` is a deterministic sha256 over **every** event in canonical sequence order (canonical content via the committed `buildCanonicalObject`/`canonicalize` **+** each stored `selfHash`, length-prefixed) — a **full-generation** fingerprint that detects an interior-row change even when count/terminal are unchanged (the required correction to a terminal-only fingerprint). Sealing is an explicit, **fail-closed** operation (`AuditSealRegistrarService`, the only seal writer): it re-derives the live snapshot and refuses to write unless every authorized value matches; seals are append-only (no update/delete). The report-only monitor gains a `SEALED` verdict — for a registered seal it recomputes the snapshot and classifies `SEALED` (exact match) or `COMPROMISED reason=sealed_generation_tampered` (any mismatch); only explicitly-sealed chainIds take this path, so a failed active chain can never become healthy. The **authorized initial `system` gen-0 seal** (`eventCount=3`, `terminalSequence=3`, `terminalSelfHash=7d24dc07…232590`, `snapshotDigest=1e5276a0…b734bd`) was **independently derived** from the three frozen rows with the production helper; `registerInitialSystemSeal` fails closed if the deployed generation does not reproduce it. **The canonical `AuditVerificationService` is unchanged** (verified); recorder/hash/chain/export untouched; no auto-sealing; no admin seal API. Commit: `feat(audit): support sealed audit generations`. |
...
| **Verification (R-016b success criteria)** | Interior tampering (count + terminal sequence + terminal selfHash unchanged, interior event changed) → `COMPROMISED` ✅; matching frozen generation → `SEALED` ✅; active `system:g1` → `VERIFIED` (not SEALED) ✅; unsealed broken chain → `COMPROMISED` ✅; appended/removed/terminal-mutation after sealing → `COMPROMISED` ✅; monitor never auto-seals ✅; sealed/compromised counts distinct + `HEALTHY` only when every chain is `VERIFIED` or `SEALED` ✅; canonical verifier still rejects the broken legacy chain directly ✅; registrar fails closed on snapshot mismatch and on absent `system` rows ✅. tsc clean; **full audit suite 34 suites / 391 tests green** (25 new). |
| **Owner** | Unassigned (backend) |
| **Operational follow-up (deploy-time)** | Run `AuditSealRegistrarService.registerInitialSystemSeal()` against each environment that carries the frozen `system` gen-0 fragment to record it `SEALED` (fails closed if the deployed rows differ from the authorized snapshot). Code-complete; this is an operational step, not an open engineering item. `system-chain-contamination.regression.spec.ts` premise already shifted to the frozen generation under R-016a. |
